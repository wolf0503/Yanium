      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-16 grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-4">
          {steps.map((s, i) => (
            ...
            >
              <span className="mb-4 block font-display text-4xl font-bold text-gold/15 select-none">
                {s.num}
              </span>
              <p className="mb-1 font-mono text-[11px] tracking-[0.3em] text-gold/60">
                // {s.num} · {s.label}
              </p>
              <p className="mb-4 font-serif text-base leading-relaxed text-silver/70">{s.body}</p>
        ...
        >
          <p className="mb-5 font-mono text-[11px] tracking-[0.3em] text-silver/40">
            // TYPICAL TIMELINES
          </p>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
            {timelines.map((t, i) => (
              <div key={i} className="flex flex-col gap-1">
                <span className="font-serif text-base text-silver/60">{t.label}</span>
                <span className="font-display text-base tracking-[0.1em] text-gold">{t.value}</span>
              </div>
            ))}
          </div>
          <div className="mt-8 border-t border-white/[0.06] pt-5">
            <span className="font-mono text-[12px] tracking-[0.15em] text-silver/30">
              1663°C — Lutetium&apos;s melting point. The highest of any rare earth. We operate at
              that temperature.
            </span>
          </div>
        </motion.div>
