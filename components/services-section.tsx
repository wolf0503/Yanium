    ...
    >
      ...
      >
        ...
        <div className="mb-5 flex items-start justify-between">
          <div
            className={`flex h-12 w-12 items-center justify-center border transition-all duration-500 ${hovered ? "border-gold/60 bg-gold/10" : "border-gold/25 bg-gold/5"}`}
          >
            <s.icon className="h-6 w-6 text-gold" />
          </div>
          <span className="font-display text-4xl font-bold text-gold/15 select-none">
            {s.pillarNum}
          </span>
        </div>

        <p className="mb-1 font-mono text-[11px] tracking-[0.3em] text-gold/55">{s.tag}</p>
        <h3 className="mb-3 font-display text-lg tracking-[0.08em] text-silver md:text-xl">
          {s.label}
        </h3>
        ...
        <ul className="mb-5 flex flex-wrap gap-2">
          {s.detail.map((d) => (
            <li
              key={d}
              className="rounded-sm border border-gold/15 px-2 py-1 font-mono text-[11px] tracking-[0.1em] text-gold/55"
            >
              {d}
            </li>
          ))}
        ...
        <div className="mt-auto border-t border-white/[0.08] pt-4">
          <span className="font-mono text-[11px] tracking-[0.15em] text-silver/35">
            {s.callout}
          </span>
        </div>
        ...
        {/* Corner accents */}
        <span
          className="pointer-events-none absolute -left-[2px] -top-[2px] h-4 w-4 border-l border-t transition-all duration-500"
          style={{ borderColor: hovered ? "rgba(212,175,55,0.5)" : "rgba(212,175,55,0.2)" }}
        />
        <span
          className="pointer-events-none absolute -bottom-[2px] -right-[2px] h-4 w-4 border-b border-r transition-all duration-500"
          style={{ borderColor: hovered ? "rgba(212,175,55,0.5)" : "rgba(212,175,55,0.2)" }}
        />
      </motion.div>
    </div>
      ...
      <div className="mx-auto max-w-6xl">
        ...
        >
          <span className="mb-4 inline-block font-display text-[12px] tracking-[0.4em] text-gold/70">
            // OXIDATION STATE +3 · THREE FORCES · ONE SYSTEM
          </span>
          ...
          >
            Lutetium&apos;s oxidation state is always +3 — stable, unified, never varies. Yanium
            operates the same way. Three forces. Every engagement. One system.
          </motion.p>
        </motion.div>
