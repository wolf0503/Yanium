  ...
  const init = useCallback(() => {
    ...
    const draw = () => {
      ...
      for (const s of streams) {
        ...
        ctx.fillStyle = `rgba(212,175,55,${s.alpha * s.progress * 0.65})`;
        ctx.fill();
      }
      ...
      for (const p of disk) {
        ...
        ctx.fillStyle = `hsla(${p.hue},${p.sat}%,${p.lit}%,${p.alpha * 0.5})`;
        ctx.fill();
      }
      ...
      for (const j of jets) {
        j.y -= j.speed;
        if (j.y < 0) {
          j.y = EH + Math.random() * j.yMax;
          j.x = (Math.random() - 0.5) * scene.EH * 0.24;
        }
        const fade = 1 - j.y / j.yMax;
        ctx.beginPath();
        ctx.arc(
          cx + j.x + Math.sin(t + j.y * 0.05) * j.drift * 3,
          cy - j.y * j.side,
          j.size,
          0,
          Math.PI * 2
        );
        ctx.fillStyle = `rgba(0,220,255,${j.alpha * fade * 0.55})`;
        ctx.fill();
      }
      ...
      for (const side of [-1, 1]) {
        ...
        jg.addColorStop(0.08, `rgba(0,240,255,${0.1 + pulse * 0.05})`);
      ...
      ctx.save();
      ctx.beginPath();
      ctx.arc(cx, cy, EH - 1, 0, Math.PI * 2);
      ctx.fillStyle = "#000";
      ctx.fill();
      ctx.restore();
      ...
      for (const p of disk) {
        ...
        ctx.beginPath();
        ctx.arc(x, y, p.size * ((1 - p.band) * 0.5 + 0.5), 0, Math.PI * 2);
      ...
      ctx.beginPath();
      ctx.arc(cx, cy, EH * 5.2, 0, Math.PI * 2);
      ctx.fillStyle = dg;
      ctx.fill();
      ctx.restore();
      ...
      ctx.beginPath();
      ctx.arc(cx, cy, EH * 1.6, 0, Math.PI * 2);
      ctx.fillStyle = haloG;
      ctx.fill();
      ...
      ctx.shadowBlur = 28;
      ctx.beginPath();
      ctx.arc(cx, cy, EH + EH * 0.07, 0, Math.PI * 2);
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const dx = e.clientX - cx,
        dy = e.clientY - cy;
      mouseRef.current.proximity = Math.max(
        0,
        1 - Math.sqrt(dx * dx + dy * dy) / (Math.min(W, H) * 0.38)
      );
    };
