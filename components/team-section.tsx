...
export function TeamSection() {
  ...
  return (
    <section id="team" className="relative py-12 px-6 md:py-32">
      ...
      <div className="mx-auto max-w-5xl">
        ...
        >
          ...
          >
            In lutetium&apos;s nucleus, 104 neutrons do the invisible work. No one sees them.
            Everything depends on them. Yanium is built the same way.
          </motion.p>
        </motion.div>
          ...
          <div className="grid grid-cols-1 divide-y divide-white/[0.05] md:grid-cols-2 md:divide-x md:divide-y-0">
            {disciplines.map((d, di) => (
              <div key={di} className="p-7">
                <p className="mb-5 font-mono text-[11px] tracking-[0.3em] text-gold/60">
                  {d.category}
                </p>
                <div className="flex flex-col gap-4">
                  {d.roles.map((r, ri) => (
                    ...
                    >
                      <span className="font-display text-base tracking-[0.08em] text-silver">
                        {r.title}
                      </span>
                      <span className="font-mono text-[11px] tracking-[0.12em] text-silver/40">
                        {r.stack}
                      </span>
                    </motion.div>
                  ))}
        ...
        >
          <p className="max-w-xl font-serif text-base leading-relaxed text-silver/55">
            We don&apos;t list headcounts. We list capabilities. Every specialist on a Yanium
            project is exactly where they need to be — not assigned by availability, but matched by
            expertise.
          </p>
