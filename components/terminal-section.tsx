...
const LINES: LineData[] = [
  ...
  {
    type: "simple",
    text: "> VERDICT: The last rare earth. The one that completes the series.",
    delay: 10000,
  },
...
function SimpleLine({ line, visible }: { line: SimpleLine; visible: boolean }) {
  ...
  return (
    <div className="flex items-center">
      <span className={`font-mono text-sm ${line.color === "gold" ? "text-gold" : "text-cyan/90"}`}>
        {shown}
      </span>
...
function MetricLine({ line, visible }: { line: MetricLine; visible: boolean }) {
          ...
          const biv = setInterval(() => {
            ...
            if (bi >= line.filled) {
              ...
              const riv = setInterval(() => {
                ...
                const v = parseFloat(
                  ((1 - Math.pow(1 - p, 3)) * line.value).toFixed(line.value % 1 !== 0 ? 2 : 0)
                );
                setVal(v);
                if (p >= 1) {
                  clearInterval(riv);
                  setVal(line.value);
                  setTimeout(() => setShowTag(true), 200);
                }
              }, 16);
            }
          }, 60);
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-2 font-mono text-sm">
      ...
      {showTag && (
        <span
          className={`text-sm font-bold ${line.tagColor === "gold" ? "text-gold" : "text-cyan"}`}
        >
          {line.tag}
        </span>
      )}
  ...
  return (
    <section id="terminal" className="relative py-12 px-6 md:py-32">
      <div className="mx-auto max-w-4xl">
        ...
        >
          <span className="mb-4 font-display text-[12px] tracking-[0.4em] text-gold/70">
            // ELEMENT DATA · Yn · 71
          </span>
          ...
          >
            ...
            <div className="flex items-center gap-2 border-b border-white/10 px-4 py-3">
              ...
              <span className="ml-3 font-mono text-sm text-silver/40">
                yanium://element-71 · Yn · 288.07
              </span>
            </div>
            <div className="flex min-h-[320px] flex-col gap-3 p-6">
              <div className="mb-2 font-mono text-sm text-silver/35">
                // ELEMENT 71 · LUTETIUM DATA — LIVE
              </div>
              {LINES.map((line, i) =>
                line.type === "simple" ? (
